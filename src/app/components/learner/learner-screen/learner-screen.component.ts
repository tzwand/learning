...
export class LearnerScreenComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // });
    debugger
    this.addService.getCurrentRequests(sessionStorage.getItem('learnerId') as unknown as number)
    .subscribe(success => { this.offers = (success), this.onSuccess() })
  }

  onSuccess() {
    if (this.offers[0] != undefined) {
      console.log("BookName:" + this.offers[0].reqPurpose + "bookname:" + this.offers[0].BookName)
  ...
  onDeleteConfirm(event) {
    ...
    this.fileNameDialogRef
      ...
      .subscribe((add) => {
      this.addLearningBoolean = add, this.canContinue = true, this.dealWithSubscribeConfirmLearning(event);
